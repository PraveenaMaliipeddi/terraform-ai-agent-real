...
export default function RealTerraformAgent() {
  ...
  const handleConfirm = async (approved) => {
    ...
    if (!approved) {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: '❌ Action cancelled. No resources created. No charges.\n\nFeel free to modify the plan!'
      }]);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.needsPermissionUpdate || (data.message && data.message === 'PERMISSION_UPDATE_REQUIRED')) {
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: '⚠️ Permission Update Required',
            ...
            <div key={idx}>
              <div className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-3xl rounded-2xl px-5 py-4 shadow-lg ${
                  ...
                  {msg.needsPermissionUpdate && (
                    <div className="mt-4 bg-amber-950/50 border border-amber-800/50 rounded-xl p-5">
                      <div className="flex items-start gap-3 mb-4">
                        <AlertTriangle className="w-6 h-6 text-amber-400 flex-shrink-0" />
                        <div>
                          <h4 className="font-semibold text-amber-200 text-lg mb-3">
                            🔧 Permission Update Required
                          </h4>
                          <p className="text-amber-100 text-sm mb-4">
                            Your connection was created before we added encryption support. You need to reconnect with updated permissions.
                          </p>
                          <div className="bg-amber-900/30 rounded-lg p-4 space-y-3 text-sm">
                            <div className="flex items-start gap-2">
                              <span className="text-amber-300 font-bold">1.</span>
                              <span className="text-amber-100">
                                <a href="https://console.aws.amazon.com/cloudformation" 
                                   target="_blank" 
                                   rel="noopener noreferrer"
                                   className="text-amber-200 hover:text-amber-100 underline">
                                  Open CloudFormation Console ↗
                                </a> and delete the "TerraformAI-Access" stack
                              </span>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-amber-300 font-bold">2.</span>
                              <span className="text-amber-100">
                                Click "Disconnect" button (top right) to clear your old connection
                              </span>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-amber-300 font-bold">3.</span>
                              <span className="text-amber-100">
                                Click "Setup Secure Connection" and create a fresh stack with updated permissions
                              </span>
                            </div>
                          </div>
                          <button
                            onClick={handleDisconnect}
                            className="mt-4 w-full bg-amber-600 hover:bg-amber-700 text-white rounded-lg py-2 font-medium transition-all"
                          >
                            Disconnect Now
                          </button>
                        </div>
